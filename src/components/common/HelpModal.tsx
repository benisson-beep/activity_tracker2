...
export const HelpModal: React.FC<HelpModalProps> = ({ isOpen, onClose, onOpenContact }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/65 backdrop-blur-sm animate-fade-in overflow-y-auto">
      <div 
        className="w-full max-w-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden my-8"
          ...
          <div className="p-6 max-h-[60vh] overflow-y-auto space-y-3">
            {faqs.map((faq, index) => {
              ...
              return (
                <div
                  key={index}
                  className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 overflow-hidden transition-all"
                >
                  <button
                    onClick={() => setOpenFaqIndex(isOpen ? null : index)}
                    className="w-full px-4 py-3 text-left flex items-center justify-between text-xs font-bold text-slate-900 dark:text-white"
                  >
                    <span>{faq.q}</span>
            ...
            <div className="pt-3 p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-center">
              ...
              <button
                ...
                className="mt-2 px-4 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs shadow-xs transition-all inline-flex items-center space-x-1.5"
              >
                <Mail size={13} />
        ...
        {activeTab === 'shortcuts' && (
          <div className="p-6 max-h-[60vh] overflow-y-auto space-y-2">
            {shortcuts.map(s => (
              <div
                key={s.key}
                className="flex items-center justify-between py-1.5 px-2 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/50 text-xs"
              >
                <span className="text-slate-600 dark:text-slate-300 font-medium">
                  {s.desc}
                </span>
                <kbd className="px-2 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-mono font-bold text-slate-800 dark:text-slate-200 text-[11px] shadow-2xs">
                  {s.key}
                </kbd>
              </div>
            ))}
          ...
          <div className="p-6 max-h-[60vh] overflow-y-auto space-y-5">
            <div className="space-y-3">
              <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700/60 flex items-center justify-between">
                <div>
                  <h4 className="text-xs font-bold text-slate-900 dark:text-white">Direct Message</h4>
                ...
                <button
                  ...
                  className="px-3.5 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold shadow-xs transition-all"
                >
                  Open Contact Form
                </button>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700/60 flex items-center justify-between">
                <div>
                  <h4 className="text-xs font-bold text-slate-900 dark:text-white">Discord Community</h4>
                ...
                <a
                  ...
                  className="px-3.5 py-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold shadow-xs transition-all inline-flex items-center space-x-1"
                >
